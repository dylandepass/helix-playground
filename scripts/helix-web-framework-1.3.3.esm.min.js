 ...
 * @param {Element} main The container element
 * @preserve
 */function f(e){const t=[...e.querySelectorAll(":scope > div.section")];for(let e=0;e<t.length;e+=1){const o=t[e];if("loaded"!==o.getAttribute("data-section-status")){if(o.querySelector('.block[data-block-status="initialized"], .block[data-block-status="loading"]')){o.setAttribute("data-section-status","loading");break}o.setAttribute("data-section-status","loaded")}}}
/**
 ...
 * @preserve
 */function k(e){const t=Array.isArray(e)?e:[e];window.hlx=window.hlx||{},window.hlx.dependencies&&Array.isArray(window.hlx.dependencies)?window.hlx.dependencies.concat(t):window.hlx.dependencies=t}class L{constructor(e){this.config=e,x(),this.config.rumEnabled&&(this.sampleRUM("top"),window.addEventListener("load",(()=>$("load"))),document.addEventListener("click",(()=>$("click")))),this.loadPage(document),window.name.includes("performance")&&v()}
/**
   * log RUM if part of the sample.
   * @param {string} checkpoint identifies the checkpoint in funnel
   * @param {Object} data additional data for RUM sample
